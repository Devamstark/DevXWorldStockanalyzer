...
def get_portfolio():
    """Get portfolio stocks with live data"""
...
def add_to_portfolio():
    ...
    if not symbol or not name:
        return jsonify({"error": "Missing symbol or name"}), 400
        
    if database.add_stock(symbol, name):
...
def remove_from_portfolio(symbol):
    ...
    return jsonify({"message": "Removed from portfolio"}), 200
